    ...
    <div class="cart-content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="content-header">
          ...
          <div class="pro-price">目前筹资/目标筹资</div>
          <div class="pro-num">投资金额</div>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          ...
          <div class="pro-img">
            <router-link :to="{ path: '/goods/details', query: {productID:item.project_id} }">
              <img :src="item.img_path" />
            </router-link>
          </div>
          <div class="pro-name">
            <router-link
              :to="{ path: '/goods/details', query: {productID:item.project_id} }"
            >{{item.name}}</router-link>
          </div>
          <div class="pro-price">{{item.raiser_amount}}元/{{item.target_amount}}元</div>
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.num"
              @change="handleChange($event,index,item.project_id)"
              :min="1"
              :max="item.max_num"
            ></el-input-number>
          </div>
          <div class="pro-total pro-total-in">{{item.num}}元</div>
            ...
            <el-popover placement="right">
              ...
              <div style="text-align: right; margin: 10px 0 0">
                <el-button
                  type="primary"
                  size="mini"
                  @click="deleteItem($event,item.id,item.project_id)"
                >确定</el-button>
              </div>
      ...
      <div class="cart-bar">
        <div class="cart-bar-left">
          <span>
            <router-link to="/goods">继续浏览</router-link>
          </span>
          <span class="sep">|</span>
          <span class="cart-total">
            共
            <span class="cart-total-num">{{getNum}}</span> 类项目，已选择
            <span class="cart-total-num">{{getCheckNum}}</span> 类
          </span>
        </div>
