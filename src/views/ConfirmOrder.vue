  ...
  <div class="confirmOrder">
    ...
    <div class="confirm-content">
      ...
      <div class="section-address">
        <p class="title">请选择众筹项目后农产品的发往地址</p>
      ...
      <div class="section-goods">
        <p class="title">请确认众筹项目信息</p>
        <div class="goods-list">
          <ul>
            <li v-for="item in getCheckGoods" :key="item.id">
              <img :src="item.img_path" />
              <span class="pro-name">{{item.name}}</span>
              <!-- <span class="pro-price">您想为此投资{{item.num}}元</span> -->
              <span class="pro-num">x1</span>
              <span class="pro-status"></span>
              <span class="pro-total">您想为此投资{{item.num}}元</span>
            </li>
          </ul>
        </div>
      </div>
    ...
    addOrder() {
      ...
      for (let i = 0; i < orders.length; i++) {
        ...
        var form = {
          ...
          //项目id
          project_id: orders[i].project_id,
          //投资金额
          num: orders[i].num,
          //送货地址id
            ...
            if (res.status === 200) {
              ...
              cartsAPI
                ...
                .then(res => {
                  if (res.status === 200) {
                    // 更新vuex状态
                    this.deleteShoppingCart(temp.project_id)
                  } else if (res.status === 20001) {
                    //token过期，需要重新登录
                    this.loginExpired(res.msg)
                  } else {
              ...
              this.notifySucceed('结算成功！未付款的订单将于15分钟后删除')
            } else if (res.status === 20001) {
              //token过期，需要重新登录
              this.loginExpired(res.msg)
            } else {
