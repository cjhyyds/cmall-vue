  ...
  <div id="details" v-if="productDetails">
    ...
    <div class="details-main">
      ...
      <div class="details-block">
        <div>
          <img style="height: 560px;" :src="productDetails.img_path" />
        </div>
      </div>
      <!-- 左侧商品轮播图END -->
      ...
      <div class="details-content">
        ...
        <div class="price">
          <span>{{ productDetails.raiser_amount }}元</span>
          <span
        ...
        <div class="pro-list">
          ...
          <span class="pro-price">
            <span>{{ productDetails.target_amount }}元</span>
            <span
              v-show="true"
              class="pro-del"
            >{{ productDetails.price }}元</span>
          </span>
          <p class="price-sum">目标筹集 : {{ productDetails.target_amount }}元</p>
          <p class="price-sum">当前筹集 : {{ productDetails.raiser_amount }}元</p>
          <p class="price-sum">发起人ID : {{ productDetails.raiser }}</p>
          <p class="price-sum">剩余天数 : {{ productDetails.remaining_time }}天</p>
          <p class="price-sum">商品名称 : {{ productDetails.name }}</p>
          <p class="price-sum">商品简介 : {{ productDetails.info }}元</p>
          <p class="price-sum">商品ID : {{ productDetails.category_id }}元</p>
        </div>
        <!-- 内容区底部按钮 -->
        <div class="button">
...
export default {
  ...
  methods: {
    ...
    addShoppingCart() {
      ...
      //如果此时已经登录，形成一个包含用户id和项目id的form表单
      var form = {
      ...
      favoritesAPI
        ...
        .then(res => {
          if (res.status === 200) {
            ...
            this.notifySucceed('您已经收藏过该项目了', res.msg)
          }
        })
        .catch(err => {
