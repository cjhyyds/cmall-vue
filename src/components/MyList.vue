  ...
  <div id="myList" class="myList">
    <ul>
      <li v-for="(item,index) in list" :key="item.id">
        ...
        <!--此处为根据商品id查找详细信息的API-->
        <router-link :to="{ path: '/goods/newdetails', query: {productID:item.id} }">
        <!--商品的图片路径-->
          ...
          <p>
            <!--商品的当前筹资-->
            <span>已经筹资{{item.raiser_amount}}元</span>
          </p>
          <p>
            <!--商品的目标筹资-->
            <span>目前筹资{{item.target_amount}}元</span>
          </p>
          <h2>还剩{{item.remaining_time}}天结束</h2>
        </router-link>
      </li>
